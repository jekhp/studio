...
export default function MapPage() {
    ...
    const regionalFestivals = upcomingFestivals.filter(f => f.isRegional);
    ...
    return (
        <div className="relative h-[calc(100vh-4rem)]">
            ...
            {regionalFestivals.length > 0 && (
                 <div className="absolute top-4 right-4 z-10 w-[90%] max-w-sm md:max-w-xs space-y-2">
                    {regionalFestivals.map(festival => (
                        <RegionalFestivalBanner key={festival.id} festival={festival} />
                    ))}
                </div>
            )}
